...
def get_data():
    """
    :return: a T * 3 list of stock prices, each row is a different stock, 0 = AAPL, 1 = MSI, 2 = SBUX
    """
...
class ReplayBuffer:
    ...
    def sample_batch(self, batch_size=32):
        ...
        return dict(s=self.obs1_buf[idx], s2=self.obs2_buf[idx], a=self.acts_buf[idx], r=self.rews_buf[idx],
                    d=self.done_buf[idx])
...
class MultiStockEnv:
    """
    a three-stock trading environment
    ...
    def step(self, action):
        # assert expression is equivalent to if not expression: raise AssertionError
    ...
    def _trade(self, action):
        ...
        # sell any stocks we want to sell and then buy any stocks we want to buy
        # NOTE: to simplify the problem,
        # when we sell, we will sell ALL shares of that stock
        # when we buy, we will loop through each stock we want to buy,
